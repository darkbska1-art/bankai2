...
try {
    ...
    |--------------------------------------------------------------------------
    | AFK komutu mu?
    | B!afk yazarken AFK'dan çıkmasın
    |--------------------------------------------------------------------------
    */
    ...
    if (guildId) {
        ...
        if (usedPrefix) {
            ...
            const commandPart = message.content
    ...
    /*
    |--------------------------------------------------------------------------
    | 1️⃣ AFK olan kişi mesaj yazarsa AFK'dan çıkar
    |--------------------------------------------------------------------------
    */
    ...
    if (
        ...
        const afkUser = afkData[guildId][userId];

        delete afkData[guildId][userId];

        if (
            Object.keys(afkData[guildId]).length === 0
        ) {
        ...
        */
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(
                ...
                `💤 **AFK süren:** ${durationText}\n` +
                `💬 **Sebebin:** ${afkUser.reason}`
            )
            .setFooter({
    ...
    /*
    |--------------------------------------------------------------------------
    | 2️⃣ AFK kişiyi etiketleyenlere bilgi ver
    |--------------------------------------------------------------------------
    */
    ...
    if (
        ...
        for (const mentionedUser of message.mentions.users.values()) {
            ...
            const mentionedAFK =
                afkData[guildId]?.[mentionedUser.id];

            if (!mentionedAFK) continue;

            const duration =
                Date.now() - mentionedAFK.since;
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(
                    ...
                    `💬 **Sebep:** ${mentionedAFK.reason}\n` +
                    `⏱️ **AFK süresi:** ${durationText}`
                )
                .setFooter({
                    text: `Bankai • ${message.guild.name}`
                })
            ...
            // Bir mesajda birden fazla AFK bildirimiyle spam yapmasın
            break;
        }
    }
